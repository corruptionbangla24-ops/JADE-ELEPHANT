...
// 🐘 ওরিজিনাল জেড এলিফ্যান্ট ৫টি লাক্সারি ক্যাসিনো সিম্বল ম্যাপ
const jadeSymbolsPool = ["ELEPHANT", "JADE_COIN", "GOLDEN_TREE", "FROG", "LOTUS"];
...
app.get('/api/slot-balance', async (req, res) => {
    ...
    try {
        const response = await axios.post(`${MAIN_SITE_URL}/api_callback.php`, {
            action: "balance", username: finalUser, amount: 0, wallet: targetWallet, game: "jadeelephant"
        }, { timeout: 15000 });
...
// 🛫 ২. জেড এলিফ্যান্ট কোর ৫-রিল স্পিন রাউট (মানি ট্রি ও ফ্যান-টানের মতো ১০০% সিকিউরড সিঙ্গেল পাইপলাইন প্রোটোকল)
app.post('/api/slot-spin', async (req, res) => {
    const { userId, amount, wallet } = req.body; 
    const reqAmount = parseFloat(amount) || 50; // ডিফল্ট ৫০ টাকা স্টেক লক চ্যাম
    const finalGameName = "jadeelephant"; 
    const targetWallet = wallet || "main";
    ...
    try {
        ...
        while (isLoopActive && loopSafety < 150) {
            ...
            for (let i = 0; i < 5; i++) {
                let randomIdx = Math.floor(Math.random() * jadeSymbolsPool.length);
                finalReelsResultMatrix.push(jadeSymbolsPool[randomIdx]);
            }
            ...
            if (maxMatchesCount === 5) {
                ...
                if (matchedSymbolName === "ELEPHANT") winMultiplier = 50.00;       
                else if (matchedSymbolName === "JADE_COIN") winMultiplier = 25.00; 
                else if (matchedSymbolName === "GOLDEN_TREE") winMultiplier = 20.00; 
                else if (matchedSymbolName === "FROG") winMultiplier = 15.00; 
                else winMultiplier = 12.00;                          
                finalStatus = "win";
            } else if (maxMatchesCount === 4) {
                winMultiplier = (matchedSymbolName === "ELEPHANT") ? 8.00 : 4.00;
                finalStatus = "win";
            } else if (maxMatchesCount === 3) {
                winMultiplier = (matchedSymbolName === "ELEPHANT") ? 2.50 : 1.50;
                finalStatus = "win";
            ...
            if (balResponse.data && balResponse.data.jadeelephant_target) {
                let target = String(balResponse.data.jadeelephant_target).toUpperCase();
                if (target === "FORCE_LOSE" && finalStatus === "win") {
                    finalReelsResultMatrix = ["ELEPHANT", "JADE_COIN", "GOLDEN_TREE", "FROG", "LOTUS"];
...
server.listen(PORT, () => { console.log(`💎 Jade Elephant Royal 5-Reel Slots Engine Running on port ${PORT}`); });
